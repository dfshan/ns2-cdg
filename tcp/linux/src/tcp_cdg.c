 ...
 *   o More accurate e^-x.
 */
/* Modified Linux module source code from Linux 4.12.10 */
#define NS_PROTOCOL "tcp_cdg.c"
...
static void tcp_enter_cwr(struct sock *sk, const int set_ssthresh, u32 in_flight)
{
 ...
 */
static void tcp_cdg_hystart_update(struct sock *sk, u32 inflight)
{
	...
	if (hystart_detect & HYSTART_ACK_TRAIN) {
		...
		u32 now_us = (u32) (ktime_get_real / 1000);

		if (ca->last_ack == 0 || !tcp_is_cwnd_limited(sk, inflight)) {
			ca->last_ack = now_us;
...
static bool tcp_cdg_backoff(struct sock *sk, u32 grad, u32 in_flight)
{
	struct cdg *ca = inet_csk_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
			      ...
			      u32 seq_rtt, u32 in_flight, int data_acked)
{
	...
	//printf("after=%d, ca->rtt.v64=%lld\n", after(ack, ca->rtt_seq), ca->rtt.v64);
...
static void tcp_cdg_acked(struct sock *sk, u32 cnt, ktime_t last)
{
	struct cdg *ca = inet_csk_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);

	u32 sample_rtt_us =  ktime_to_us(net_timedelta(last));
	if ((s32) sample_rtt_us <= 0)
		return;
	// printf("last=%d\n", last);
	 ...
	 */
	// TODO: how to filtering delayed ACKs in ns-2
	//if (tp->sacked_out == 0) {
	//	if (cnt == 1 && ca->delack) {
	//		/* A delayed ACK is only used for the minimum if it is
	//		 * provenly lower than an existing non-zero minimum.
	//		 */
	//		ca->rtt.min = min(ca->rtt.min, sample_rtt_us);
	//		ca->delack--;
	//		return;
	//	} else if (cnt > 1 && ca->delack < 5) {
	//		ca->delack++;
	//	}
	//}

	//printf("u8:%lu, u16:%lu, u32:%lu, u64:%lu, s32:%lu, s64:%lu\n", sizeof(u8), sizeof(u16), sizeof(u32), sizeof(u64), sizeof(s32), sizeof(u64));
	ca->rtt.min = min_not_zero(ca->rtt.min, sample_rtt_us);
	ca->rtt.max = max(ca->rtt.max, sample_rtt_us);
}
